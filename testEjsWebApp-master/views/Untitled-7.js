...
//饮食偏好
    ...
    // 全身症状
    spirit:{'spirit_jinglichongpei': '精力充沛',
            'spirit_jianwang': '健忘',
            'spirit_jingshenbujizhong': '精神不集中',
            'spirit_shenpifali': '神疲乏力',
            'spirit_yalida': '学习、工作压力大',
...
for (var i in archiveobjs.geninfo) {
    if (excel_key.hasOwnProperty(i)){// i in excel_key
        excel_key[i] = archiveobjs.geninfo[i];
    } else {
        for (var j in excel_key) {
            if (typeof(excel_key[i])!="object") {
                excel_key[i] = archiveobjs.geninfo[i];
            } else {
                excel_key[i][j]  
            }
